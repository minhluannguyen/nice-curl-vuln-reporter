...
- `internet_access` is optional and is set to `false` by default to ensure that the VM does not have access to the internet during the test execution, which is important for security and reproducibility. If set to `true`, the VM will have internet access, but it is recommended to keep it disabled unless the test scenario specifically requires it since it can introduce uncontrolled vulnerability factors and affect the reproducibility of the tests.
